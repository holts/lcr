...
void set(unsigned int x,unsigned int y,unsigned char c)	//»­Ò»¸öµã  
{	LCD_SetXY(x,y);
	DrPoint(c);
}
...
void D57A(unsigned int x,unsigned int y,unsigned char n,unsigned char c)	//Êä³öÒ»¸ö5*7×Ö·û
{	unsigned char i,j,v;
	unsigned int nn;
	n=n-32;
	nn=n*5;
...
void LCD_WDA57(unsigned int x, unsigned int y,unsigned char *string,unsigned char c) //x:x×ø±ê y:y×ø±ê dd:×Ö·û
{
	unsigned int i=0;  
	unsigned int j=0;   
...
void D12HZ(unsigned int x,unsigned int y,unsigned char n,unsigned char c)	//Êä³öÒ»¸ö12µãÕóºº×Ö
{	unsigned char i,j,v;
	n=n-0x80;
	for (i=0;i<12;i++)
	{  	v=hz12[n][i];
...
void LCD_WHZ12(unsigned int x, unsigned int y,unsigned char *string,unsigned char c) //x:x×ø±ê y:y×ø±ê dd:×Ö·û
{
	unsigned char i=0;  
	unsigned char j=0;   
...
void D48ASC(unsigned int x,unsigned int y,unsigned char n,unsigned char c)	//Êä³öÒ»¸ö47µãÕóÊý×Ö
...
void LCD_WHZ48(unsigned int x, unsigned int y,unsigned char *string,unsigned char c) //x:x×ø±ê y:y×ø±ê dd:×Ö·û
{
	unsigned char i=0;  
	unsigned char j=0;   
